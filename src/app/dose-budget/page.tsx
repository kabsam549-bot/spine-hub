...
function BudgetResult({ result }: { result: OARBudgetResult }) {
  ...
  const customPhysicalDose = customFx && customFx >= 1
    ? eqd2ToPhysicalDose(result.remainingBudgetEQD2, customFx, result.oar.alphaBeta)
    : undefined;
  ...
  return <div className={`mt-4 rounded-2xl border ${tone.border} ${tone.bg} p-4`}>
    ...
    <div className="mt-4 overflow-hidden rounded-xl border border-gray-200 bg-white/80"><div className="border-b border-gray-200 px-3 py-2 text-[11px] font-bold uppercase tracking-wide text-gray-500">Physical dose budget</div><table className="w-full text-sm"><thead className="text-left text-[11px] uppercase tracking-wide text-gray-500"><tr><th className="px-3 py-2">Fractions</th><th className="px-3 py-2">Total dose</th><th className="px-3 py-2">Dose/fx</th></tr></thead><tbody className="divide-y divide-gray-200">{physicalRows.map((row) => <tr key={row.fractions}><td className="px-3 py-2 font-semibold text-gray-800">{row.fractions} fx</td><td className="px-3 py-2 font-semibold text-gray-900">{row.totalDose.toFixed(1)} Gy</td><td className="px-3 py-2 text-gray-600">{row.dosePerFraction.toFixed(1)} Gy</td></tr>)}<tr><td className="px-3 py-2"><input type="number" min="1" max="50" value={customFractions} onChange={(event) => setCustomFractions(event.target.value)} className="w-14 rounded border border-gray-300 px-2 py-1 text-center text-xs" placeholder="#" /> <span className="text-xs text-gray-500">fx</span></td><td className="px-3 py-2 font-semibold text-gray-900">{customPhysicalDose !== undefined ? `${customPhysicalDose.toFixed(1)} Gy` : "-"}</td><td className="px-3 py-2 text-gray-600">{customPhysicalDose !== undefined && customFx ? `${(customPhysicalDose / customFx).toFixed(1)} Gy` : "custom"}</td></tr></tbody></table></div>
    <details className="mt-4 rounded-xl border border-gray-200 bg-white/80"><summary className="cursor-pointer px-3 py-2 text-xs font-bold text-gray-600">Calculation details</summary><div className="space-y-3 border-t border-gray-200 px-3 py-3 text-xs text-gray-600"><div className="rounded-lg bg-white p-3 font-mono leading-relaxed"><p>d = total dose ÷ fractions</p><p>BED = D × (1 + d ÷ α/β)</p><p>EQD2 = BED ÷ (1 + 2 ÷ α/β)</p></div><div className="overflow-x-auto rounded-lg border border-gray-200 bg-white"><table className="min-w-full text-left"><thead className="bg-gray-50 text-[10px] font-bold uppercase tracking-wide text-gray-500"><tr><th className="px-2 py-2">Course</th><th className="px-2 py-2">Actual OAR dose</th><th className="px-2 py-2">d/fx</th><th className="px-2 py-2">BED</th><th className="px-2 py-2">EQD2</th></tr></thead><tbody className="divide-y divide-gray-100">{result.courseBreakdown.map((course) => <tr key={course.courseIndex}><td className="px-2 py-2 font-semibold text-gray-800">{course.courseIndex}</td><td className="px-2 py-2">{course.dose.toFixed(1)} Gy / {course.fractions} fx</td><td className="px-2 py-2">{course.dosePerFraction.toFixed(2)} Gy</td><td className="px-2 py-2">{course.bed.toFixed(1)} Gy</td><td className="px-2 py-2 font-semibold text-gray-900">{course.eqd2.toFixed(1)} Gy</td></tr>)}</tbody></table></div><div className="rounded-lg bg-white p-3 font-mono leading-relaxed"><p>Prior EQD2 = {result.courseBreakdown.map((course) => course.eqd2.toFixed(1)).join(" + ")} = {result.priorEQD2.toFixed(1)} Gy</p><p>Remaining = {result.effectiveTolerance.toFixed(1)} − {result.priorEQD2.toFixed(1)} = {result.remainingBudgetEQD2.toFixed(1)} Gy EQD2</p><p>Physical-dose conversion uses the remaining EQD2 and α/β = {result.oar.alphaBeta} Gy.</p></div>{result.oar.specialNote && <p className="rounded-lg bg-blue-50 p-3 leading-relaxed text-blue-900">{result.oar.specialNote}</p>}</div></details>
  </div>;
}
